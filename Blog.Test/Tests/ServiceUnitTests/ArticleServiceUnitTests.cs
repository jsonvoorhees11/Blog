    ...
    public class ArticleServiceUnitTests{
        ...
        //Arrange
            ...
            var options = new DbContextOptionsBuilder<BlogDbContext>()
                            ...
                            .Options; 
            IMapper<Article, ArticleDto> mapper = SetupMapperMocking();

            using (var context = new BlogDbContext(dataSeedingMock.Object,options)){
                context.Articles.AddRange(articles);
                context.SaveChanges();
            }

            using (var context = new BlogDbContext(dataSeedingMock.Object,options)){
                var service = new ArticleService(context,mapper);
                var articleCount = service.GetArticles().Count();
                Assert.True(articleCount>0);
            }
        }
        ...
        private IMapper<Article, ArticleDto> SetupMapperMocking(){
            var mapperMock = new Mock<IMapper<Article, ArticleDto>>();
            mapperMock.Setup(m=>m.MapEntityToDto(It.IsAny<Article>())).Returns(new ArticleDto());
